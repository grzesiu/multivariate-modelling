...
The number of components that optimize the value of `adjCV` is (-1 because the first model has no components, only intercept):
...
And the error rate obtained with it is:

```{r}
min(preds$error)
```

While the error rate obtained with the number of components that has maximized the R2 is:
