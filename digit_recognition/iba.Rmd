...
Nevertheless, we will try with bigger number of them in order to see, how does IBA deal with multicollinearity.
...
getCoefs <- function(Y.one.hot, x.t) {
  return(solve(t(x.t) %*% x.t) %*% t(x.t) %*% Y.one.hot)
}
```
...
On the plot it can be seen, that IBA method is doing very well with collinearity. The error rate drops from 1 to 9 components and after that remains almost constant.
...
Here, as in the previous section, it can be observed that collinearity is not a problem for this method.
...
- it deals well with collinearity
can be confirmed.
